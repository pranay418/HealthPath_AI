...
# Initialize form states for inputs to support auto-fill and manual corrections
if "form_sugar" not in st.session_state:
    st.session_state["form_sugar"] = 0.0
if "form_hba1c" not in st.session_state:
    st.session_state["form_hba1c"] = 0.0
if "form_hemoglobin" not in st.session_state:
    st.session_state["form_hemoglobin"] = 0.0
if "form_cholesterol" not in st.session_state:
    st.session_state["form_cholesterol"] = 0.0
if "form_bp" not in st.session_state:
    st.session_state["form_bp"] = 0.0
...
if menu == "Analyze Report":
    ...
    st.markdown("Upload medical lab reports (PDF/Images) to auto-fill metrics, or enter values manually. You can review and correct any pre-filled value before generating your plan.")
    
    # Optional file uploader for auto-filling form
    file = st.file_uploader("Upload Lab Report PDF or Image (Optional)", type=["png", "jpg", "jpeg", "pdf"])
    
    if file:
        file_key = f"processed_{file.name}_{file.size}"
        # Process OCR only once per upload to optimize speed
        if st.session_state["last_processed_file"] != file_key:
            with st.spinner("Analyzing report and auto-filling metrics..."):
                ...
                if parsed_data:
                    # Update session state values (binds directly to widget keys)
                    if "sugar" in parsed_data:
                        st.session_state["form_sugar"] = parsed_data["sugar"]
                    if "hba1c" in parsed_data:
                        st.session_state["form_hba1c"] = parsed_data["hba1c"]
                    if "hemoglobin" in parsed_data:
                        st.session_state["form_hemoglobin"] = parsed_data["hemoglobin"]
                    if "cholesterol" in parsed_data:
                        st.session_state["form_cholesterol"] = parsed_data["cholesterol"]
                    if "bp" in parsed_data:
                        st.session_state["form_bp"] = parsed_data["bp"]
                    
                    st.success("✅ Form pre-filled from report! Please review the values below, correct any mistakes, and click 'Generate Health Plan'.")
                else:
                    st.warning("⚠️ Could not extract specific lab values automatically. Please verify and enter your metrics manually below.")
                
                # Show extracted raw OCR text for transparency/troubleshooting
                with st.expander("🔍 View Extracted OCR Text"):
                    st.text(text)
                
                st.session_state["last_processed_file"] = file_key

    # -------- Unified Form (Allows manual typing and corrections) --------
    st.markdown("### 📝 Verify & Adjust Lab Metrics")
    col1, col2 = st.columns(2)
    with col1:
        sugar = st.number_input("Fasting Blood Sugar (mg/dL)", min_value=0.0, key="form_sugar", step=5.0)
        hba1c = st.number_input("HbA1c (%)", min_value=0.0, key="form_hba1c", step=0.1)
        hemoglobin = st.number_input("Hemoglobin (g/dL)", min_value=0.0, key="form_hemoglobin", step=0.5)
    with col2:
        cholesterol = st.number_input("Total Cholesterol (mg/dL)", min_value=0.0, key="form_cholesterol", step=10.0)
        bp = st.number_input("Blood Pressure (Systolic)", min_value=0.0, key="form_bp", step=5.0)
    ...
    if st.button("🚀 Generate Health Plan", type="primary", use_container_width=True):
        ...
        if sum(data.values()) == 0:
            st.error("Please enter at least one health metric before generating the schedule.")
        else:
